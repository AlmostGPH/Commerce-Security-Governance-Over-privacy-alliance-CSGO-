...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        --weekly) weekly="$2"; shift ;;
        --monthly) monthly="$2"; shift ;;
        --yearly) yearly="$2"; shift ;;
        --env) env="$2"; shift ;;
...
mkdir -p "$save_path/scripts"

touch $save_path/scripts/data_man.py

cat > $save_path/scripts/data_man.py <<EOF
'''
...
$env/python3 $save_path/scripts/data_man.py --weekly $weekly --monthly $monthly --yearly $yearly
